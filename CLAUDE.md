...
- **Each year discounts at the rate for the horizon still remaining, not the plan's original one.** Quantile dispersion shrinks as 1/sqrt(T), so one full-horizon rate is only ~1,7 points under the median on a forty-year plan — and stays 1,7 points under it in year 38, when two years left deserves nearer 7. That made the reserve most optimistic exactly where a bad sequence can no longer be recovered from, and it was the rule's main source of aggression: fixing it took the default plan from 84% to 86% survival for 0,9% of the expected spending, the recovered share of the survival gap from 70% to 81%, and the final year's failure step from 1,67x its predecessor to 1,44x. Nothing else tried came close on that exchange rate — measured in survival points per mkr of expected spending given up: 27 for this, 24 for a bequest target, 21 for spending only 70% of the surplus, 14 for a p10 quantile at the full horizon, and **1,6 for scaling the extra**, which is the solver's lever and by far the worst, because it cuts discretionary spending in the good states too, where it buys no safety. A commitment therefore reaches year 0 through one-year factors that run from long horizons to short ones, rather than being discounted at the rate for its own distance; that stationary alternative — the path sitting at the 25th percentile at *every* horizon at once — is more conservative again and untried.
...
- **The survival curve steepens towards the horizon, and that part is real.** The annuity factor falls to one in the final year, so the surplus is spent roughly twice as fast each of the last few years and failures accelerate with it — the last step is about 1,44x its predecessor on the default plan (AF 1,51x) and it tracks the horizon rather than a calendar year. Anything sharper than that is either the depleted outcome above being miscounted or the reserve rate having gone back to a single full-horizon figure, which took the same step to 1,67x.
