...
│   ├── stress.ts                - Good/plan/bad scenarios across stress sliders
...
- **The final-distribution chart marks the median, not the peak.** A mode is only a summary while the density is smooth, and the atom above is the tallest thing on that chart whenever a target is set — so marking the peak labelled the *target*, a property of the rule rather than of the plan, however unlikely the plan was to land there. The median comes straight off `outcomes[years].median`, the same figure the Slutkapital table prints and on the same net-of-tax basis, so the two cannot disagree; `densityAt` is only for the marker's height, since the results carry no density and the dot has to sit on the drawn curve. No marker at all when the median is zero, which is a plan that fails more than half the time and which a log axis cannot show.
...
### The stress test (`stress.ts`, `StressCard.vue`)

One card, a slider per assumption, and three columns: the plan as entered with everything stressed the good way, the plan itself, and everything stressed the bad way. Rows match the Slutkapital table exactly, so the two read against each other without translation.

- **All stressed dimensions move together, in one direction.** That is the whole reason this replaced a pair of two-axis grids: a plan rarely fails because a single assumption was wrong, and the combinations are what a grid cannot show. It is a deliberately *correlated* scenario, not a probability — nothing claims the assumptions move together, only that this is the cost if they do.
- **Each dimension declares which way hurts** (`worse: 'higher' | 'lower'`). Without that there is no single good-to-bad axis: more need is worse, less return is worse, and they have to point the same way to share a column.
- **The slider is geometric, not linear.** A position `s` gives a factor of `2^±s`, so full is doubled or halved and the two directions are *reciprocals at every position*. On an arithmetic scale +50% and −50% are not the same move, and the good and bad columns would be off the plan by different proportions. Half-way is therefore 1,41x, which is why each slider states its own factors rather than trusting "50%" to explain itself.
- **Zero means untouched**, and all sliders start there: the card opens on the plan and lets the household say what it doubts. With nothing stressed the three columns would be one number printed three times, so the table is replaced by a prompt and *nothing is computed* — including no busy state, which would otherwise flash on every clear.
- **The previous figures stay under a spinner while the next ones compute.** Dropping them empties the card for most of a second on an AF plan, exactly while a reader is dragging a slider and needs the thing they were comparing against. The overlay is what makes that honest — stale figures under a spinner read as being replaced, stale figures alone read as current — and `aria-busy` carries the same to a screen reader. The spinner is indeterminate because the propagations are synchronous and hold the thread, so a progress bar would have to invent its own progress.
- **Three propagations, not nine.** Cheap enough to recompute on a slider drag behind a 350 ms debounce, where the two 9-cell grids were not.
- **Dimensions:** need, extra, expected real return, volatility, schablon rate, and — for AF only — inflation. Returns and volatility are separate because scaling both would confound "I am wrong about what this earns" with "the market is a different market". The schablon rate is there because it is the model's largest sensitivity (see the assumptions section above) and was until now un-surfaced. The bequest target is deliberately absent: it is a preference the household set, not an assumption it might have wrong.
- **A dimension can decline to apply** (`applies`), and `stressDimensionsFor` is what the card and the engine both go through. Levels for a dimension that does not apply are ignored everywhere, so switching account type cannot leave a hidden stress in force behind a slider that is no longer on screen.
- **Inflation is offered to an AF and withheld from an ISK.** Everything is real and the schablon proportional, so inflation only reaches the result where the tax code is written in nominal kronor. An AF's omkostnadsbelopp is not indexed, so a rising price level is taxed as gain: doubling inflation costs the default plan 4 points of survival and a fifth of its final capital. An ISK has only the fribelopp, and the same move is worth 0,3 points and nothing measurable in capital — under the resolution the outputs are rounded to. A slider that cannot move the answer reads as a broken model rather than as a plan that does not depend on a forecast nobody can make, so it is filtered out rather than shown doing nothing. Both halves are pinned by tests: the ISK claim is a statement about the model, and a regression that made it matter should not pass silently just because the slider is hidden.
- **The middle column keeps its own units** under the relative switch: it is the reference the other two are stated against, and three zeroes there would delete the anchor. Survival moves in **percentage points** (`formatPointChange`) while the money rows are relative — reporting a probability as a ratio invites "fell by a quarter" to be read as "fell by 25 points".

### Collapsible cards (`CollapsibleCard.vue`)

The stress test, the computation table and *Om metoden* all fold away behind their headers, all closed by default, all on this one component.
...
- **A button in the header, not `<details>`/`<summary>`.** The stress card carries a switch in the header beside the title, and a control nested inside a `<summary>` swallows its own clicks into the disclosure and reads as one interactive thing to a screen reader.
- **`open` is a model, not internal state**, because a card may need to know: the stress test computes only while open, and closing it has to stop the work rather than merely hide it. It tracks `show.bs.collapse`/`hide.bs.collapse`, which fire when the animation *starts*, so the work begins as the card opens rather than a third of a second later.
