...
- **The survival curve steepens towards the horizon, and that part is real.** The annuity factor falls to one in the final year, so the surplus is spent roughly twice as fast each of the last few years and failures accelerate with it — the last step is about 1.7x its predecessor on the default plan, and it tracks the horizon rather than a calendar year. Anything sharper than that is not the rule but the depleted outcome above being miscounted.
